...
void wait_for_pin_high(uint pin, uint debounce_us) {
    gpio_init(pin);
    gpio_set_dir(pin, GPIO_IN);
    gpio_pull_down(pin);

    LOG_INFO("Awaiting trigger (debounce = %u us).", debounce_us);
    ...
    while (true) {
        ...
        if (gpio_get(pin) == 1) {
            break;   // valid high state
        }
        // else it was a glitch – continue waiting
    }
...
int main() {
    ...
    while (true) {
        ...
        if (current_msg_type == MSG_PWM) {
            ...
            wait_for_pin_high(TRIGGER_PIN, 1000);
